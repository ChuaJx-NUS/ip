...
This plan covers the user-visible task creation, display, and persistence
behavior of BigBrother. Inputs are sent one command per line. Expected output
entries below are the important task-response lines from the console transcript;
startup and separator lines are also captured and shown when the tests are run.
...
## Test 11: Save and load tasks across restarts

### Aim

Verify that adding and marking tasks saves all task details, and that a new
BigBrother process loads them from `data/bigbrother.txt`. Run both sessions from
...
## Test 12: Handle a corrupted data file
